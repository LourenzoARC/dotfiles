...
def gen_scheme(scheme, primary: Hct) -> dict[str, str]:
    ...
    # REGRAS MINIMALISTAS (PRETO OLED + VERMELHO SANGUE PURO / PURE RED)
    if scheme.variant == "tonalspot" and not is_light:
        colours["background"] = hex_to_hct("000000")
        colours["surface"] = hex_to_hct("000000")
        colours["surfaceDim"] = hex_to_hct("000000")
        colours["surfaceContainer"] = hex_to_hct("050505")
        colours["surfaceContainerHigh"] = hex_to_hct("0D0D0D")
        colours["surfaceContainerHighest"] = hex_to_hct("151515")
        colours["onBackground"] = hex_to_hct("FFFFFF")
        colours["onSurface"] = hex_to_hct("FFFFFF")
        colours["onSurfaceVariant"] = hex_to_hct("CCCCCC")

        # Se for tom de vermelho/magenta (Hue perto do topo 310-360 ou 0-40), força exatamente Vermelho Vivo Puro (24.0)
        target_hue = primary.hue
        if target_hue > 310 or target_hue < 40:
            target_hue = 24.0

        vibrant_chroma = max(primary.chroma * 2.5, 95.0)

        # Tone em 45.0 mantêm a cor viva e profunda sem desbotar para rosa
        colours["primary"] = Hct.from_hct(target_hue, vibrant_chroma, 45.0)
        colours["primaryContainer"] = Hct.from_hct(target_hue, vibrant_chroma * 0.85, 18.0)
        colours["onPrimary"] = hex_to_hct("FFFFFF")
        colours["onPrimaryContainer"] = Hct.from_hct(target_hue, vibrant_chroma * 0.85, 90.0)

        colours["secondary"] = Hct.from_hct(target_hue, vibrant_chroma * 0.75, 55.0)
        colours["secondaryContainer"] = Hct.from_hct(target_hue, vibrant_chroma * 0.65, 25.0)

...
def get_smart_opts(wall: Path, cache: Path) -> dict:
    ...
    try:
        opts_data = json.loads(opts_cache.read_text())
        # Garante que se o cache antigo não tiver 'is_dark', ele recalcule
        if "is_dark" in opts_data:
            return opts_data
    ...
    with Image.open(get_thumb(wall, cache)) as img:
        ...
        # Mede o brilho médio da imagem (0.0 = preto total, 1.0 = branco total)
        gray_img = img.convert("L")
        avg_brightness = ImageStat.Stat(gray_img).mean[0] / 255.0
        
        # Considera um wallpaper escuro se o brilho médio for menor que 35%
        opts["is_dark"] = avg_brightness < 0.35

        img_1x1 = img.copy()
        img_1x1.thumbnail((1, 1), Image.Resampling.BOX)

        # Cast the pixel to a tuple of 3 integers to safely unpack it
        pixel = cast(tuple[int, int, int], img_1x1.getpixel((0, 0)))
...
def update_mouse_led(colours: dict) -> None:
    try:
        primary = colours.get("primary")
        if primary:
            clean_color = str(primary).lstrip("#")
          
            subprocess.run(
                ["openrgb", "-m", "Direct", "-c", clean_color],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL
            )
    except Exception:
...
def get_colours_for_wall(wall: Path | str, no_smart: bool) -> dict:
    ...
    if not no_smart:
        ...
        # REGRA REFINADA: Usa 'hard' apenas se for monocromático E escuro
        is_mono = smart_opts.get("variant") == "monochrome"
        is_dark = smart_opts.get("is_dark", False) and smart_opts.get("mode") == "dark"
        
        flavour = "hard" if (is_mono and is_dark) else "default"
...
def set_wallpaper(wall: Path, no_smart: bool) -> None:
    ...
    if scheme.name == "dynamic" and not no_smart:
        ...
        # REGRA REFINADA:
        # Se for monocromático E o wallpaper for predominantemente escuro/preto -> 'hard'
        # Se for monocromático mas com fundo claro/branco -> 'default'
        is_mono = scheme.variant == "monochrome"
        is_dark = smart_opts.get("is_dark", False) and scheme.mode == "dark"

        if is_mono and is_dark:
            scheme.flavour = "hard"
        else:
    ...
    update_mouse_led(scheme.colours)
